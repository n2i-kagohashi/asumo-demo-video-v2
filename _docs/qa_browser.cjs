...
(async()=>{
  ...
  const credentials=process.env.QA_USER?{username:process.env.QA_USER,password:process.env.QA_PASS||''}:undefined;
  const context=await browser.newContext({httpCredentials:credentials});const page=await context.newPage();
  await page.setViewportSize({width:1440,height:1040});
  const errors=[];page.on('pageerror',e=>errors.push(e.message));
  await page.goto(process.env.QA_URL||'http://127.0.0.1:4322/',{waitUntil:'networkidle'});
